...
function App() {
  ...
  const handleBrowse = async () => {
    ...
    try {
      ...
      console.log('\nFRONTEND:');
      console.log(`Browse response received: ${path}`);
      if (path) {
        setFolderPath(path);
  ...
  const handleOrganize = async () => {
    ...
    try {
      ...
      if (success) {
        setFiles(prev => prev.map(f => ({ ...f, status: 'Organized' as const })));
  ...
  return (
    <div className="min-h-screen bg-[#030712] relative overflow-x-hidden text-slate-200 font-sans">
      ...
      <div className="relative z-10 w-full max-w-[1440px] mx-auto h-screen flex flex-col p-5 sm:p-6 lg:p-8 gap-5 lg:gap-6">
        ...
        <div className="flex-1 min-h-0 flex flex-col lg:flex-row gap-5 lg:gap-6 animate-fade-in-up pb-2" style={{ animationDelay: '0.4s' }}>
